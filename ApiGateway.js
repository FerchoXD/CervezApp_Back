...
router.route('/product/search').get(limiter, async(req,res ) => {
  const channel = await createConnection();

  let queueRequest = "busqueda" ; 
  let queueResponse = "busquedaRespuesta"

  const message = { 
    id :  req.body.id
  }

  const sent = await channel.sendToQueue(queueRequest, Buffer.from(JSON.stringify(message)), { persistent: true})
  ...
  try {
    ...
    res.status(500).send({error : "Ocurrio un error en la prueba"});
    socketSearch.emit('badSearch', {status : 500, message : "Ocurrio un problema en la busqueda"})
  }
})
